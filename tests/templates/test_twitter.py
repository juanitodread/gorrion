...
class TestTweetTemplate:
    def test_constructor(self, album, tweet_config):
        template = TweetTemplate(album, tweet_config)

        assert template._album == album
        assert template._config == tweet_config

    def test_to_tweet_with_header_only(self, album, tweet_song_config):
        tweet_song_config.with_header = True
        tweet_song_config.with_body = False
        tweet_song_config.with_footer = False
        template = TweetTemplate(album, tweet_song_config)

        assert template.to_tweet() == 'Now listening 🔊🎶:\n\n'

    def test_to_tweet_with_body_only(self, album, tweet_song_config):
    ...
    def test_to_tweet_with_footer_only(self, album, tweet_song_config):
        tweet_song_config.with_header = False
        tweet_song_config.with_body = False
    ...
    def test_to_tweet_default_config(self, album, tweet_config):
        template = TweetTemplate(album, tweet_config)

        assert template.to_tweet() == ''

    def test_header_with_header_true(self, album, tweet_song_config):
        tweet_song_config.with_header = True
        template = TweetTemplate(album, tweet_song_config)

        assert template.header() == 'Now listening 🔊🎶:\n\n'

    def test_header_with_header_false(self, album, tweet_song_config):
        tweet_song_config.with_header = False
        template = TweetTemplate(album, tweet_song_config)

        assert template.header() == ''

    def test_body_with_body_true(self, album, tweet_song_config):
        tweet_song_config.with_body = True
        template = TweetTemplate(album, tweet_song_config)
    ...
    def test_body_with_body_false(self, album, tweet_song_config):
        tweet_song_config.with_body = False
        template = TweetTemplate(album, tweet_song_config)

        assert template.body() == ''

    def test_footer_with_footer_true(self, album, tweet_song_config):
        tweet_song_config.with_footer = True
        template = TweetTemplate(album, tweet_song_config)
    ...
    def test_footer_with_footer_false(self, album, tweet_song_config):
        tweet_song_config.with_footer = False
        template = TweetTemplate(album, tweet_song_config)
...
class TestBodyTemplate:
    def test_constructor(self, album, tweet_song_config):
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template._album == album
        assert body_template._config == tweet_song_config.body_config

    def test_to_body_with_track_only(self, album, tweet_song_config):
    ...
    def test_to_body_with_album_only(self, album, tweet_song_config):
    ...
    def test_to_body_with_artists_only(self, album, tweet_song_config):
        tweet_song_config.body_config.with_track = False
        tweet_song_config.body_config.with_album = False
    ...
    def test_to_body_with_tracks_only(self, album, tweet_song_config):
        tweet_song_config.body_config.with_track = False
        tweet_song_config.body_config.with_album = False
        tweet_song_config.body_config.with_artists = False
    ...
    def test_to_body_with_release_date_only(self, album, tweet_song_config):
        tweet_song_config.body_config.with_track = False
        tweet_song_config.body_config.with_album = False
        tweet_song_config.body_config.with_artists = False
        tweet_song_config.body_config.with_tracks = False
    ...
    def test_body_with_track_true(self, album, tweet_song_config):
        tweet_song_config.body_config.with_track = True
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.track() == 'Track: 1. Peligro\n'

    def test_body_with_track_false(self, album, tweet_song_config):
        tweet_song_config.body_config.with_track = False
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.track() == ''

    def test_body_with_album_true(self, album, tweet_song_config):
        tweet_song_config.body_config.with_album = True
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.album() == 'Album: Pa morirse de amor\n'

    def test_body_with_album_false(self, album, tweet_song_config):
        tweet_song_config.body_config.with_album = False
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.album() == ''

    def test_body_with_artists_true(self, album, tweet_song_config):
        tweet_song_config.body_config.with_artists = True
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.artists() == 'Artist: Ely Guerra\n'

    def test_body_with_artists_false(self, album, tweet_song_config):
        tweet_song_config.body_config.with_artists = False
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.artists() == ''

    def test_body_with_tracks_true(self, album, tweet_song_config):
        tweet_song_config.body_config.with_tracks = True
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.tracks() == 'Tracks: 19\n'

    def test_body_with_tracks_false(self, album, tweet_song_config):
        tweet_song_config.body_config.with_tracks = False
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.tracks() == ''

    def test_body_with_release_date_true(self, album, tweet_song_config):
        tweet_song_config.body_config.with_release_date = True
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.release_date() == 'Release: 2006\n'

    def test_body_with_release_date_false(self, album, tweet_song_config):
        tweet_song_config.body_config.with_release_date = False
        body_template = BodyTemplate(album, tweet_song_config.body_config)

        assert body_template.release_date() == ''

    def test_to_body_default_config(self, album, tweet_config):
        body_template = BodyTemplate(album, tweet_config.body_config)
...
class TestFooterTemplate:
    def test_constructor(self, album, tweet_config):
        footer_template = FooterTemplate(album, tweet_config.footer_config)

        assert footer_template._album == album
        assert footer_template._config == tweet_config.footer_config

    def test_to_footer_with_hashtags_only(self, album, tweet_config):
    ...
    def test_to_footer_with_gorrion_hashtags_only(self, album, tweet_config):
        tweet_config.footer_config.with_gorrion_hashtags = True
    ...
    def test_to_footer_with_album_hashtag_only(self, album, tweet_config):
    ...
    def test_to_footer_with_artists_hashtag_only(self, album, tweet_config):
        tweet_config.footer_config.with_gorrion_hashtags = False
        tweet_config.footer_config.with_album_hashtag = False
        tweet_config.footer_config.with_artists_hashtag = True
        tweet_config.footer_config.with_song_media_link = False
    ...
    def test_to_footer_with_song_media_link_only(self, album, tweet_config):
        tweet_config.footer_config.with_gorrion_hashtags = False
        tweet_config.footer_config.with_album_hashtag = False
        tweet_config.footer_config.with_artists_hashtag = False
    ...
    def test_to_footer_with_album_media_link_only(self, album, tweet_config):
        tweet_config.footer_config.with_gorrion_hashtags = False
        tweet_config.footer_config.with_album_hashtag = False
        tweet_config.footer_config.with_artists_hashtag = False
        tweet_config.footer_config.with_song_media_link = False
    ...
    def test_to_footer_default_config(self, album, tweet_config):
        tweet_config.footer_config.with_gorrion_hashtags = False
        tweet_config.footer_config.with_album_hashtag = False
        tweet_config.footer_config.with_artists_hashtag = False
        tweet_config.footer_config.with_song_media_link = False
    ...
    def test_build_hashtag(self, album, tweet_config):
        footer_template = FooterTemplate(album, tweet_config.footer_config)
        ...
        assert footer_template._build_hashtag('thIS iS a AM vAlID') == '#ThisIsAAMValid'
