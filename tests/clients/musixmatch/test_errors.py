...
class TestErrors:
    ...
    def test_lyric_not_found_error(self):
        ...
        assert str(error) == 'Lyric not found. You may try another index: track_id=123, common_track_id=456'

    def test_lyric_not_provided_yet(self):
        error = LyricNotProvidedYet('123', '456')

        assert str(error) == ('Lyric not provided for this song or song is instrumental. You may try another '
                              'index: track_id=123, common_track_id=456')
